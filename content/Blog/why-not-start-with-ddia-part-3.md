...
在分布式数据系统中，最常见的应用场景是 Key-Value 存储。对于这类系统，单次读写操作通常就是一个完整的逻辑单元，因此系统的可靠性主要依赖于副本机制来保障：只要一个副本成功写入，就可以视为数据持久化，进一步副本同步由系统后台完成。

但当应用的复杂性提升，单个读写已无法满足需求时，就需要将多个读写操作**绑定为一个原子性操作单元**。这就是**事务**的概念：整个事务要么全部成功（提交），要么在任何一步失败时全部回滚。对开发者而言，这大大简化了错误处理逻辑，不必担心“部分成功”的中间状态。

### 分布式事务的 ACID 与 BASE

传统事务遵循 ACID 原则：

* **A（Atomicity，原子性）**：事务中的操作要么全部完成，要么全部不做，不会处于部分完成状态。
* **C（Consistency，一致性）**：事务完成后，系统应从一个一致状态转变为另一个一致状态。
* **I（Isolation，隔离性）**：多个事务并发执行时，相互之间不会干扰，表现得就像是串行执行。
* **D（Durability，持久性）**：事务一旦提交，其结果应永久保存，不会因故障丢失。

然而在分布式环境下，追求严格 ACID 的代价非常高，尤其是隔离性和一致性的实现容易导致可用性下降或系统性能瓶颈。因此，许多分布式系统采用了更宽松的 **BASE 理论**：

* **Basically Available（基本可用）**：系统允许在某些情况下部分可用，而不是完全宕机。
* **Soft state（软状态）**：系统状态允许在一段时间内不一致。
* **Eventual consistency（最终一致性）**：经过一段时间，系统所有副本最终会达到一致状态。

**ACID 更适合强一致性场景**，而 **BASE 更偏向于可用性和扩展性**，代表了分布式系统中的一种妥协。


### 事务隔离级别

事务的隔离性决定了并发操作时系统对读写冲突的处理方式。理想状态下，所有事务都串行执行，但这显然无法满足性能需求。因此，系统通常提供多种隔离级别，以平衡一致性与并发性：

* **读已提交（Read Committed）**：一个事务只能读取到其他事务已经提交的数据，避免脏读，但仍可能出现不可重复读。

* **快照隔离（Snapshot Isolation）**：每个事务在开始时看到的是数据库的一个一致性快照，避免了脏读和不可重复读，但可能出现写倾斜问题。

* **可重复读（Repeatable Read）**：一个事务中多次读取同一数据总能得到相同结果，防止了不可重复读，但仍可能遇到幻读（即新插入的行被“看到”）。

  * **写倾斜（Write Skew）**：多个事务在不冲突地读取后进行写操作，可能违反业务约束。
  * **幻读（Phantom Read）**：一个事务中两次相同的查询，第二次返回了第一遍没有的记录（如新增或删除导致的结果集变化）。

* **串行化（Serializable）**：最强的隔离级别，保证事务像串行执行一样运行，消除了上述所有并发问题，但实现复杂且性能开销大。常见实现有两种方式：

  * 基于锁（如两阶段锁协议 2PL）
  * 基于版本控制与检测（如 SSI：Serializable Snapshot Isolation）
